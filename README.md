...
- **HCLK** : HCLK is derived from SYSCLK with a Prescaler in between , which brings down the clock frequency. HCLK goes directly to AHB bus, core, memory and DMA. HCLK goes to Cortex System Timer with (/8) prescalar in between and HCLK goes directly to Cortex Processor (FCLK Cortex Clock).
